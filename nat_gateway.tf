...
resource "aws_eip" "loko_eip1" {
  domain = "vpc"

  tags = {
    Name = "${var.project_name}-${var.environment}-loko_eip1"
  }
}
...
resource "aws_nat_gateway" "loko_ng_az1" {
  allocation_id = aws_eip.loko_eip1.id
  subnet_id     = aws_subnet.loko_subnet_az1.id

  tags = {
    Name = "${var.project_name}-${var.environment}-loko_ng_az1"
  }

  # to ensure proper ordering, it is recommended to add an explicit dependency
  # on the internet gateway for the vpc
  depends_on = [aws_internet_gateway.loko_igw]
}
...
resource "aws_route_table" "private_routetable_az1" {
  ...
  route {
    ...
    nat_gateway_id = aws_nat_gateway.loko_ng_az1.id
  }
...
resource "aws_route_table_association" "private_app_subnet_az1_rt_az1_association" {
  subnet_id      = aws_subnet.private_lokoapp_subnet_az1.id
...
resource "aws_route_table_association" "private_data_subnet_az1_rt_az1_association" {
  subnet_id      = aws_subnet.private_lokoapp_subnet_az2.id
  route_table_id = aws_route_table.private_routetable_az1.id
}
...
resource "aws_route_table_association" "private_app_subnet_az2_rt_az2_association" {
  subnet_id      = aws_subnet.private_lokodata_subnet_az1.id
  route_table_id = aws_route_table.private_routetable_az1.id
}
...
resource "aws_route_table_association" "private_data_subnet_az2_rt_az2_association" {
  subnet_id      = aws_subnet.private_lokodata_subnet_az2.id
  route_table_id = aws_route_table.private_routetable_az1.id
}